...
const UploadButton = ({Icon, inputRef, accept, onChange, title, iconClass }) => (
    <label className='inline-flex items-center gap-2 px-3 py-2 bg-zinc-900/80 border border-zinc-700/50 rounded-xl text-sm text-zinc-200 shadow-inner cursor-pointer shrink-0 self-start' title={title}>
        ...
        <div className='bg-linear-to-br from-zinc-900/90 to-zinc-800/90 border border-zinc-700/50 rounded-2xl p-4 backdrop-blur-sm shadow-2xl sm:p-6'>
            <form onSubmit={onSubmit}>
                <div className='relative'>
                    <textarea value={prompt} onChange={(e) => onPromptChange(e.target.value)} placeholder='Pergunte qualquer Coisa... Eu posso ajudar você, contruindo, otimizando e explorando seu código.' className='w-full bg-transparent border-none outline-none text-zinc-200 placeholder-zinc-500 resize-none text-sm leading-relaxed min-h-15 max-h-27.5 focus:placeholder-zinc-600 transition-colors sm:text-base sm:min-h-20' onKeyDown={(e) => e.key === 'Enter' && (e.metaKey || e.ctrlKey) && onSubmit(e)}></textarea>

                    <div className='mt-3 mb-2 flex flex-row items-center gap-3 flex-wrap'>
                        {isVisionModel && (
                            <UploadButton Icon={FaImage} inputRef={imageInputRef} accept='image/*' onChange={onImageChange} title='Attach Image' iconClass='text-blue-300'/>
                        )}

                        {isVisionModel && (
                            <UploadButton Icon={FaFileAlt} inputRef={fileInputRef} accept='.txt, .md, .markdown, .json, .csv, .log, .yaml, .yml, .xml' onChange={onFileChange} title='Attach file (Nova only)' iconClass='text-amber-300'/>
                        )}
                    ...
                    <div className='flex flex-col justify-between pt-4 border-t border-zinc-700/50 gap-3 sm:flex-row sm:items-center sm:gap-0'>
                        ...
                        <div className='flex items-center gap-2 w-full sm:w-auto'>
                            ...
                            <button type='submit' disabled={disableSubmit} className='flex-1 px-4 py-2 bg-linear-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 disable:from-zinc-700 disable:to-zinc-800 disable:opacity-50 border border-zinc-700 disable:border-zinc-700 rounded-xl text-white font-medium transition-all duration-200 shadow-lg hover:shadow-xl disable:cursor-not-allowed'>
                                {loading ? (
                                    <div className='flex items-center justify-center gap-2'>
